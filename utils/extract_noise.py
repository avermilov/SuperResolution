...
def extract(from_path: str, extract_to_path: str, denoise_strength: int, window_size: int, kernel_size: int,
        ...
        for (batch, _) in tqdm(loader):
            for data in batch:
                ...
                if extract_type == "noise":
                    ...
                    cv2.imwrite(
                        extract_to_path + f'{i:06}_s{denoise_strength:02}w{window_size:02}k{kernel_size:02}.png',
                        cv2.cvtColor(np.round(extracted_noise + 128).astype(np.uint8), cv2.COLOR_RGB2BGR))
                elif extract_type == "denoise":
                    cv2.imwrite(
                        extract_to_path + f"{i:06}_denoised_s{denoise_strength:02}w{window_size:02}k{kernel_size:02}.png",
...
if __name__ == "__main__":
    ...
    with open(args.json) as file:
        ...
        extract_type = data["type"].lower()

    extract(from_path, to_path, noise_level, window_size, kernel_size, extract_type)
