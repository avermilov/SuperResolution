...
class TruncatedVGG19(nn.Module):
    ...
    def __init__(self, i, j):
        ...
        assert maxpool_counter == i - 1 and conv_counter == j, "One or both of i=%d and j=%d are not valid choices " \
                                                               "for the VGG19!" % (
                                                                   i, j)
    ...
    def forward(self, input):
        ...
        """
        output = self.truncated_vgg19(input.float())  # (N, feature_map_channels, feature_map_w, feature_map_h)

        return output


class IgnatNet(nn.Module):
    def __init__(self, resnet):
        super(IgnatNet, self).__init__()
